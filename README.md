...
|---:|---:|---:|---:|
| 16 | 157.6 ms | 313.4 ms | **2.0×** |
| 32 | 234.7 ms | 1,009.7 ms | **4.3×** |
...
> 위 값은 **집계 행의 값**입니다(산정기가 실제로 쓰는 값). 정상상태 구간만 잘라 비교하면
> 1장 conc=32 의 p95 가 162.6 ms 까지 내려가 배수가 6.6× 가 됩니다 — 1장 쪽은 앞 2분의
> 과도 구간이 p95 를 끌어올리는 반면 8장 쪽은 그렇지 않기 때문입니다(위 4번).
> **어느 정의로 봐도 현상은 같고, SLA 판정에 쓰이는 것은 행의 값이므로 4.3× 를 씁니다.**
