...
정상상태 기준(부하 시작 후 앞 120초 제외):

| 카드당 동시성 | 2장 | 4장 | 8장 | 8장 반복 |
|---:|---:|---:|---:|---|
| 16 | 102.8% | 107.9% | 109.1% | 2회, 폭 0.04% |
| 32 | 105.7% | 100.1% | 102.0% | 2회, 폭 2.4% |
| 64 | 105.5% | 100.7% | **75~91%** | 5회, 폭 22.1% |

![카드 확장 효율](analysis/plots/03-card-scaling.png)

**중간 동시성에서는 8장까지 선형이거나 그 이상입니다.** 카드당 동시성이 높을 때만 떨어집니다.

> ⚠ **8장 카드당 64 는 단일 값으로 쓸 수 없습니다.** 같은 조건을 **서버 재기동을 사이에 두고**
> 다섯 번 재니 74.7 / 80.8 / 83.3 / 89.1 / 91.2% 로 나옵니다(평균 83.8%). 다섯 값이 고르게
> 퍼져 있어 "이상치 하나" 가 아닙니다.
>
> **그리고 이 변동은 부하에 따라 커집니다** — 1장 conc=32 는 0.92%, 8장 카드당 16 은 0.04%,
> 8장 카드당 64 는 22.1% 입니다. **저부하 셀은 소수점까지 믿을 수 있고, 고부하 셀은 방향만
> 취하고 크기는 범위로 읽어야 합니다.**
>
> 이 표를 처음 쓸 때는 창 길이를 안 맞춘 전체평균이었고, 그래서 4장이 100% 아래로 떨어져
> 보였습니다. 정상상태로 맞추니 사라졌습니다 — **아래 "측정이 틀렸던" 목록의 한 항목입니다.**
...
## 측정이 틀렸던 일곱 가지 — 그리고 셋은 원인까지 틀렸습니다

이 프로젝트에서 가장 시간을 쓴 부분입니다. 일곱 다 **그럴듯한 숫자를 냈기 때문에** 두 번 재보지 않았다면 그대로 결론이 됐을 것들입니다.
...
기전은 모릅니다(라우터 큐잉? 공유 HTTP 경로?). **2장·4장을 같은 조건으로 쟀고 곡선이 나왔습니다** — TTFT p95 배수가 카드당 16 에서 1.46 / 1.94 / 1.99배, 카드당 32 에서 1.90 / 2.47 / 6.27배로 **카드 수를 따라 단조 증가**합니다. 처리량 효율이 아홉 칸 중 여덟에서 100% 이상인 것과 대비됩니다.

**그래서 서술이 바뀝니다: "지연이 전이되지 않는다" 가 아니라 "처리량은 전이되는데 지연만 전이되지 않는다" 입니다.**

→ 고치지 못했습니다. **한계 절에 명시하고, 다중 카드 배포의 지연 SLA 는 이 도구의 답을 그대로 믿지 마십시오.**

---

### 7. 고부하 숫자를 한 번 재고 소수점까지 인용했습니다 — 미해결

앞의 여섯은 **측정 방법**이 틀린 것이었습니다. 이건 다릅니다. 방법이 옳아도 **한 번 잰
고부하 숫자는 그 자체로 못 믿습니다.**

같은 조건(8장 카드당 64, 입력 512)을 **서버 재기동을 사이에 두고** 다섯 번 쟀습니다.
전부 같은 규약(첫 요청 기준 180~270초 고정 창), 같은 구성(엔진 8개 각각 `npuNpe0-3`,
`Device mesh` 줄까지 동일):

| 기동 | tok/s | 확장 효율 | TTFT p95 |
|---|---:|---:|---:|
| C | 7,142.9 | 74.7% | 1,028 ms |
| A | 7,730.5 | 80.8% | 800 ms |
| D | 7,965.5 | 83.3% | 691 ms |
| E | 8,517.6 | 89.1% | 777 ms |
| B | 8,719.2 | 91.2% | 1,129 ms |

**처리량 폭 22.1%, TTFT p95 폭 63%.** 다섯 값이 고르게 퍼져 있어 "이상치 하나" 로 볼 수 없습니다.
...
**이것이 무엇을 무너뜨렸나.** 이 문서에는 한동안 "8장 배포의 요청 처리율 천장이 약
61 건/초" 라고 적혀 있었습니다. 근거는 카드당 32 에서 60.94, 64 에서 60.89 라는 두 점이
평평하다는 것이었습니다. **그 두 점이 한 기동에서 나온 것이었고, 다른 기동에서는
62.4 / 66.3 으로 계속 오릅니다.** 천장이 아니라 우연한 일치였습니다.

**어떻게 갈랐나.** 처음 설계는 "같은 기동 안에서 두 번 재자" 였습니다. **그러면 답이 안
나옵니다** — 두 측정을 가른 축이 run 이 아니라 **기동**이기 때문입니다. 같은 기동 안 반복은
"run 간 변동은 아니다" 라는 배제 하나만 삽니다. 검증 세션이 그 구분을 지적해 **사이에
재기동을 넣게** 했고, 그래서 22% 가 나왔습니다.

→ **원인은 모릅니다.** 저부하가 0.04% 로 재현되므로 측정 하네스나 창 처리의 문제는
아닙니다. **읽는 규칙만 정했습니다: 저부하 셀은 소수점까지, 고부하 셀은 방향만 취하고
크기는 반복 횟수·폭과 함께 범위로.**
...
| 배치 | TTFT 제한 없음, 사용자당 ≥5 tok/s | 112명 | **9장** | 처리량 |

> **빠름 등급의 카드당 상한이 32 가 아니라 16 입니다.** 카드당 32 에서 사용자당 속도의
> 중앙값이 **29.6~30.0 tok/s** 로 30 을 못 넘습니다. 그 조건에서
> **요청의 61.7% 가 30 tok/s 에 미달합니다**(같은 조건 5회, 12,750건). 집계 처리량을
...
> 배치 등급은 **TTFT 제한이 없다는 조건에서 형식적으로 통과**하는 값입니다.
> 그리고 **다중 카드 배포에서는 이 지연이 더 나빠집니다**(아래 한계 참조).
>
> 이 칸은 세션 4 에서 **250명·6장에서 112명·9장으로 바뀌었습니다.** 8장 고부하 측정
> 셋이 들어가면서 확장 효율 추정이 내려갔기 때문입니다. **고부하 셀이 기동마다 22%
> 흔들리므로(아래 7번) 이 값은 앞으로도 움직일 수 있습니다.**
...
~~**8장 배포는 총 요청 처리율이 약 61 건/초에서 멈춥니다.**~~ **이 주장은 철회했습니다.** 근거였던 두 점(카드당 32 에서 60.94, 64 에서 60.89 건/초)은 **한 서버 기동에서 우연히 일치한 것**이었습니다. 다른 기동에서는 62.4 / 66.3 건/초로 계속 오르고, 다섯 기동 전체로는 **55.8~68.1 건/초로 흩어집니다**(위 7번). 평평해 보이는 두 점을 천장으로 읽은 것이 오류였습니다.

**남는 것은 이것입니다**: 카드당 동시성 16 에서 8장은 1장보다 카드당 9.1% 빠르고(초선형), 카드당 64 에서는 75~91% 로 떨어집니다. **방향은 견고하지만 떨어지는 지점의 크기는 기동마다 다릅니다.**

**KV 캐시는 광고된 용량의 절반쯤에서 대기열이 생깁니다.** 서버가 로그에 찍는 `max_kv_len` 은 263,329 토큰인데, 스케줄러가 그 **40~47%** 에서 승인을 멈춥니다. 메모리가 부족해서가 아니라 스케줄러가 더 승인하지 않는 것이고, 원인은 확인하지 못했습니다.

**11개 조건에서 확인했습니다** — 잘린 조건의 `kv_cache_peak` 이 0.396~0.472 이고 안 잘린 조건(0.040~0.388)과 **겹치지 않습니다.** 토큰 수로 보면 114,560~133,120 으로 16% 흔들리는데 **KV 점유율로 보면 좁은 띠에 들어옵니다** — 불변량은 토큰 수가 아니라 점유율입니다. **다만 상한이 입력 길이를 따라 단조 상승하고(512 에서 0.397, 8,192 에서 0.472) 그 이유는 모릅니다.** 참고로 **KV 캐시는 FP16 입니다** — 가중치가 FP8 이라고 KV 도 FP8 로 가정하면 수용량을 2배로 잡습니다.
...
2장·4장 곡선은 확보했고 **카드 수를 따라 단조 증가**합니다. 기전은 여전히 미상입니다.

**측정 범위**
- 모델은 `Qwen3-8B-FP8` 이 주 대상입니다. `Qwen3-4B-FP8` 은 prefill 계단 구조만 확인했고 용량 곡선은 창이 짧아 비교 대상이 아닙니다
- 입력 512 의 절벽은 128↔256, 1,024 는 64↔128, 2,048 은 32↔64 입니다. **셋 다 "요청 총토큰이 KV 승인 상한을 넘는 지점" 이라는 한 현상입니다** — 입력이 길수록 더 적은 동시성에서 상한에 닿으므로 절벽이 앞으로 옵니다. 추세가 아니라 산수입니다
- 승인 상한은 토큰 수가 아니라 **KV 점유율**입니다. 잘린 조건 11개가 전부 `kv_cache_peak` 0.396~0.472 에 들어오고 안 잘린 조건(0.040~0.388)과 겹치지 않습니다. **다만 상한이 입력 길이를 따라 단조 상승하며(512 0.397 → 8,192 0.472) 이유는 모릅니다**
...
- 8장 카드당 16 의 확장 효율이 **109.1% 로 초선형**입니다. 카드 차이·유지율·in-flight·프롬프트 길이·DP 라우팅을 배제했지만 **기전을 모릅니다.** 2장·4장을 같은 조건으로 쟀고(102.8 / 107.9 / 109.1) **카드 수를 따라 단조 증가**하는 것까지 확인했지만 이유는 여전히 미상입니다
- **고부하 셀은 서버 기동마다 흔들립니다.** 같은 조건·같은 구성인데 재기동만으로 8장 카드당 64 의 처리량이 22% 움직입니다. **한 번 잰 고부하 숫자를 소수점까지 인용하지 마십시오**

**측정하지 않은 것**
- **혼합 부하 간섭** — 실제 서비스는 prefill과 decode가 섞여 들어옵니다. 긴 문서 하나가 들어올 때 다른 사용자 응답이 얼마나 느려지는지 측정하지 않았습니다(러너와 config 는 준비돼 있습니다)
- ~~**8장 배포의 요청 처리율 천장** — 총 약 61 건/초~~ **철회했습니다.** 근거였던 두 점(카드당 32 에서 60.94, 64 에서 60.89)이 **한 서버 기동에서 우연히 일치한 것**이었습니다. 다른 기동에서는 62.4 / 66.3 건/초로 계속 오르고, 다섯 기동 전체로는 55.8~68.1 건/초로 흩어집니다. **천장이 아니라 기동 간 변동이었습니다**
- **8장 카드당 128 (총 동시성 1,024)** — 두 번 시도해 두 번 다 못 쟀습니다. 클라이언트와 서버 양쪽 파일 디스크립터 한도가 리눅스 기본값 1,024 라 그 동시성에 닿기 전에 터집니다. **기계 한계가 아니라 설정입니다**
