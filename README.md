...
과도 구간 아티팩트가 아닙니다 — 60초 구간으로 갈라도 전 구간이 평탄합니다. 그리고 두 조건 모두 `peak_running / 요청 = 1.00` 입니다. **서버는 요청한 동시성을 다 물고 있는데 첫 토큰만 늦습니다.**
...
```bash
python -m analysis.process              # raw 측정 → 집계 테이블 (87행)
